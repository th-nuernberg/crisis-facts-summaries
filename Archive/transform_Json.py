...
def newinput():
    file="D:\\Meine Dateien\\Uni\\IT-Projekt\\Arbeit\\it-projekt\\website\\Data\\Datensaetze\\008" #\\2018-07-30.json"
    file_out="D:\\Meine Dateien\\Uni\\IT-Projekt\\Arbeit\\it-projekt\\website\\Data\\Datensaetze\\Datensatz_8.json"
    for f in os.listdir(file):
        df = pd.read_json(file+'\\'+f, orient='records')
        test = df[['event','doc_id', 'source_type', 'unix_timestamp',  'text']].to_dict('records')
        df.insert(4, 'lang', "en")
        df.insert(5, 'meta', "")
        df.insert(6, 'title', "")
